var gameCompleteState = (function() {
    ...
    return {
        ...
        create: function() {
            ...
            againBtn = game.add.button(WIDTH/2 - 120, HEIGHT - 30, 'again', this.restartGame, this);
            againBtn.anchor.setTo(0.5, 1);
            againBtn.scale.set(scaleX, scaleY);

            shareBtn = game.add.button(WIDTH/2 + 120, HEIGHT - 30, 'share', this.share, this);
            shareBtn.anchor.setTo(0.5, 1);
            shareBtn.scale.set(scaleX, scaleY);

            word = game.add.image(WIDTH/2 - 40, HEIGHT - 120, 'word');
            word.anchor.setTo(0.5, 0.5);
            word.scale.set(scaleX, scaleY);

            timeCost = game.add.image(WIDTH/2, 105, 'timeWord');
            timeCost.anchor.setTo(0.5, 0.5);
            timeCost.scale.set(scaleX, scaleY);

            timeText = game.add.text(WIDTH/2 - 30, 110, time + 's', { fontSize: '42px', fill: '#da8546' });
        },
        update: function() {
